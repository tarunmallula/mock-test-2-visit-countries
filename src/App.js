...
class App extends Component {
  ...
  onChangeStatus = id => {
    this.setState(prevState => ({
      initialList: prevState.initialList.map(eachCountry => {
  ...
  render() {
    ...
    const visitedList = initialList.filter(each => each.isVisited === true)
    const length = visitedList.length > 0
    return (
      <div className="container">
        <div className="app-container">
          ...
          {length ? (
            ...
            <p className="no-items">No Countries Visited Yet!</p>
          )}
        </div>
      </div>
    )
